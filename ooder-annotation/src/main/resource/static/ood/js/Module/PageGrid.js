ood.Class('ood.Module.PageTreeGrid', 'ood.Module',{
    Instance:{
        _mode:"normal",
        ...
        iniComponents : function(){
            ...
            append(
                ...
                .setHost(host,"ood_msgs1")
                .onMessageReceived("_ood_msgs1_onmessagereceived")
            );
            ...
            append(
                ...
                .setZIndex(1002)
                .setImageClass("oodcon ood-refresh")
        ...
        propSetAction:function(prop){
            var module=this;
            if(module._innerModulesCreated && module.ood_msgs1){
                if('inMsgType' in prop) module.ood_msgs1.setRecipientType(prop.inMsgType);
            }
        },
        loadTreeGridData:function(page, size){
            var ns=this, 
                grid=ns.grid,
                cb=function(data){
                    if(data.totalCount){
                    ...
                    if(data.rows && data.columns){
                        ...
                        ood.arr.each(data.rows,function(cells){
                            var row={};
            ...
            ood.Dom.busy("Getting Data ...");
            ns.fireEvent("onListRecords", [page, size, cb]);
            ns.ood_msgs1.broadcast(ns.properties.outMsgType, "list",  page, size, cb);
        },
        ...
        deleteRows:function(ids){
            ...
            ood.message("You deleted "+ids.length+" record(s)!");
            ood.Dom.free();
        },
        _openForm:function(recordId, fields){
            var ns = this;
            var prop={};
            if(ood.isSet(recordId)){
                ns.fireEvent("onOpenRecord",[recordId,fields,ns.updateRow]);
                ns.ood_msgs1.broadcast(ns.properties.outMsgType, "open",  recordId, fields, ns.updateRow);
            }else{
                ns.fireEvent("onCreateRecords",[ns.addRow, ns.updateRow]);
                ns.ood_msgs1.broadcast(ns.properties.outMsgType, "create",  '', ns.updateRow, ns.addRow);
            }
        },
        _delRecords:function(ids){
            ...
            ood.Dom.busy("Deleting Data ...");
            ns.fireEvent("onDeleteRecords", [ ids, cb]);
            ns.ood_msgs1.broadcast(ns.properties.outMsgType, "delete",  ids, '', cb);
        },
        _toolbar_onclick:function (profile, item, group, e, src){
            ...
            switch(item.id){
                ...
                case "delete": 
                    ...
                    if(ids&&ids.length){
                        ood.confirm("Confirm", "Are you sure you want to delete these "+ids.length+" records?", function(){
                            ns._delRecords(ids);
                        });
                    }else{
                        ood.message("You have to select some rows first!");
                    }
                    break;
                          }
        },
        ...
        _ctl_sbutton1_onclick:function (){
            ...
            this.loadTreeGridData(pb.getPage(), pb.getPageCount());
        },
        _pagebar_onpageset:function (profile, page, start, size, eventType, opage, ostart){
            this.loadTreeGridData(page,size); 
        },
        _ood_msgs1_onmessagereceived:function (profile, msg1, msg2, msg3, msg4, msg5, callback){
