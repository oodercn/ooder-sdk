...
public class LlmConnection {
     ...
     * 同步对话
     */
    public LlmDriver.ChatResponse chat(LlmDriver.ChatRequest request) {
        checkAvailable();
        try {
            updateLastUsedTime();
            CompletableFuture<LlmDriver.ChatResponse> future = driver.chat(request);
            return future.get();
        } catch (Exception e) {
     ...
     * 异步对话
     */
    public CompletableFuture<LlmDriver.ChatResponse> chatAsync(LlmDriver.ChatRequest request) {
        checkAvailable();
        updateLastUsedTime();
     ...
     * 流式对话
     */
    public CompletableFuture<LlmDriver.ChatResponse> chatStream(LlmDriver.ChatRequest request, LlmDriver.ChatStreamHandler handler) {
        checkAvailable();
        updateLastUsedTime();
