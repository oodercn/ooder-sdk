...
public class SparkLlmDriver extends AbstractLlmDriver {
    ...
    protected CompletableFuture<CompletionResponse> doComplete(CompletionRequest request) {
        ...
        DriverChatRequest chatRequest = DriverChatRequest.builder()
                .messages(Arrays.asList(ChatMessage.user(request.getPrompt())))
                .build();
    ...
    protected CompletableFuture<ModelInfo> doGetModelInfo(String modelId) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            info.setModelName("Spark " + modelId);
     ...
     * 创建模拟响应（用于测试）
     */
    private DriverChatResponse createMockChatResponse(DriverChatRequest request) {
