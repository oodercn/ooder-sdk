...
import net.ooder.skills.store.SkillStore;
...
public class SkillRegistryImpl implements SkillRegistry {
    ...
    public List<InstalledSkill> getInstalledSkills() {
        ...
        for (Map.Entry<String, SkillDefinition> entry : definitions.entrySet()) {
            ...
            InstalledSkill installed = new InstalledSkill();
            installed.setSkillId(skillId);
            installed.setName(def.getSkillId());
            installed.setVersion(def.getMetadata().getOrDefault("version", "unknown").toString());
            installed.setSceneId(def.getSceneId());
            installed.setStatus(service != null && service.isRunning() ? "RUNNING" : "STOPPED");
            installed.setInstallTime(registerTimes.getOrDefault(skillId, 0L));
    ...
    public String getLatestVersion(String skillId) {
        ...
        if (def != null && def.getMetadata() != null) {
            Object version = def.getMetadata().get("latestVersion");
            if (version != null) {
                return version.toString();
            }
            version = def.getMetadata().get("version");
            if (version != null) {
                return version.toString();
            }
        }
