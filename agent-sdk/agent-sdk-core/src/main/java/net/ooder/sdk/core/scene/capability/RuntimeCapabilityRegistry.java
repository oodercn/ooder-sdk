...
public class RuntimeCapabilityRegistry {
    ...
    private final Map<String, List<RuntimeCapabilityConsumer.RuntimeCapabilityListener>> listeners;
    ...
    public void subscribe(String capabilityId, RuntimeCapabilityConsumer.RuntimeCapabilityListener listener) {
        listeners.computeIfAbsent(capabilityId, k -> new ArrayList<>()).add(listener);
    }
    
    public void unsubscribe(String capabilityId, RuntimeCapabilityConsumer.RuntimeCapabilityListener listener) {
        List<RuntimeCapabilityConsumer.RuntimeCapabilityListener> list = listeners.get(capabilityId);
        if (list != null) {
    ...
    private void notifyListeners(String capabilityId, String event, RuntimeCapability capability) {
        List<RuntimeCapabilityConsumer.RuntimeCapabilityListener> list = listeners.get(capabilityId);
        if (list != null) {
            for (RuntimeCapabilityConsumer.RuntimeCapabilityListener listener : list) {
                try {
