...
import net.ooder.sdk.skill.driver.impl.InterfaceParserImpl;
    ...
    public void execute() {
        try {
            ...
            InterfaceDefinition interfaceDef = parser.parse(
                new File(interfacePath).toURI().toURL().openStream()
            );
            
            GeneratorOptions options = new GeneratorOptions();
            options.setCategory(interfaceDef.getSceneId());
            options.setVersion(interfaceDef.getVersion());
            options.setPackageName("net.ooder.scene.skills." + interfaceDef.getSceneId().toLowerCase());
