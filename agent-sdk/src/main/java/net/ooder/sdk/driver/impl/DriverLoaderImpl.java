package net.ooder.sdk.driver.impl;
...
public class DriverLoaderImpl implements DriverLoader {
    ...
    private static final Logger log = LoggerFactory.getLogger(DriverLoaderImpl.class);
    ...
    public DriverLoaderImpl(InterfaceRegistry interfaceRegistry) {
        ...
        if (interfaceRegistry != null) {
            ...
            if (defOpt.isPresent()) {
                ...
                if (driverConfig != null && driverConfig.getFallback() != null) {
                    ...
                    log.debug("Attempting to load fallback driver: {} -> {}", interfaceId, fallbackSkillId);
                    return load(interfaceId, fallbackSkillId, interfaceType);
                }
            }
        }
    ...
    public DriverLoaderStats getStats() {
        ...
        int fallbackCount = 0;
        if (interfaceRegistry != null) {
            for (InterfaceDefinition def : interfaceRegistry.getAllInterfaces()) {
                if (def.getDriver() != null && def.getDriver().getFallback() != null) {
                    fallbackCount++;
                }
            }
        }
        stats.setFallbackDrivers(fallbackCount);
