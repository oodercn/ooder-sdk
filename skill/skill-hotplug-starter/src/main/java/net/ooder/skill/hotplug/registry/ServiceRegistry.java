...
public class ServiceRegistry {
    ...
    public void registerService(String skillId, ServiceDefinition serviceDef, PluginClassLoader classLoader) {
        logger.info("[Service注册] 开始注册服务: {} (skill: {})", serviceDef.getName(), skillId);
        logger.debug("[Service注册][{}] 接口类: {}, 实现类: {}", 
                serviceDef.getName(), serviceDef.getInterfaceClass(), serviceDef.getImplementationClass());

        try {
            // 加载接口和实现类
            logger.debug("[Service注册][{}] 步骤1/4: 加载接口类", serviceDef.getName());
            Class<?> interfaceClass = classLoader.loadClass(serviceDef.getInterfaceClass());
            logger.debug("[Service注册][{}] 接口类加载成功: {}", serviceDef.getName(), interfaceClass.getName());
            
            logger.debug("[Service注册][{}] 步骤2/4: 加载实现类", serviceDef.getName());
            Class<?> implClass = classLoader.loadClass(serviceDef.getImplementationClass());
            logger.debug("[Service注册][{}] 实现类加载成功: {}", serviceDef.getName(), implClass.getName());

            // 创建服务实例
            logger.debug("[Service注册][{}] 步骤3/4: 创建服务实例", serviceDef.getName());
            Object serviceInstance = createServiceInstance(implClass);
            
            if (serviceInstance == null) {
                logger.warn("[Service注册][{}] 服务实例创建失败，跳过注册 (可能需要Spring注入)", serviceDef.getName());
                return;
            }
            logger.debug("[Service注册][{}] 服务实例创建成功: {}", serviceDef.getName(), serviceInstance.getClass().getName());

            // 创建动态代理
            logger.debug("[Service注册][{}] 步骤4/4: 创建动态代理", serviceDef.getName());
            Object proxy = createProxy(interfaceClass, serviceInstance, classLoader, skillId);
            logger.debug("[Service注册][{}] 动态代理创建成功", serviceDef.getName());
            ...
            logger.info("[Service注册] 服务注册成功: {}.{} (接口: {})",
                    skillId, serviceDef.getName(), interfaceName);

        } catch (ClassNotFoundException e) {
            logger.error("[Service注册] 类加载失败 - 服务: {}, skill: {}, 错误: {}", 
                    serviceDef.getName(), skillId, e.getMessage(), e);
            throw new RuntimeException("Service registration failed - Class not found: " + e.getMessage(), e);
        } catch (Exception e) {
            logger.error("[Service注册] 服务注册失败: {} (skill: {}), 错误: {}", 
                    serviceDef.getName(), skillId, e.getMessage(), e);
