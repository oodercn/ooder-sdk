/**
 * $RCSfile: HttpClientInvocationHandler.java,v $
 * $Revision: 1.0 $
 * $Date: 2025/08/25 $
 * <p>
...
public class HttpClientInvocationHandler implements InvocationHandler {
    ...
    public final static String jdstime = "jdstime";
    ...
    private static final Log logger = LogFactory.getLog(JDSConstants.CONFIG_KEY, HttpClientInvocationHandler.class);
    ...
    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
        clientService = getCurrJDSClient();
        ...
        if (clientService.getSessionHandle() != null) {
            sessionId = clientService.getSessionHandle().getSessionID();
        }

        if (clientService.getConnectInfo() == null) {
            throw new JDSException("session失效,请重新登录！", JDSException.NOTLOGINEDERROR);
        }
        ...
        if (!url.contains("?")) {
            url = url + "?" + JDSContext.JSESSIONID + "=" + sessionId;
        } else {
            url = url + "&" + JDSContext.JSESSIONID + "=" + sessionId;
        }

        String userId = clientService.getConnectInfo().getUserID();
        String syscode = clientService.getSystemCode();
        url = url + "&" + JDSContext.SYSCODE + "=" + syscode + "&" + JDSContext.JDSUSERID + "=" + userId + "&" + jdstime + "=" + System.currentTimeMillis();
...

        {
            ...
            for (Object annotation : annotationa) {
                if (RequestBody.class.isAssignableFrom(annotation.getClass())) {
                    ...
                    logger.debug(JSONObject.toJSONString(obj, config));
