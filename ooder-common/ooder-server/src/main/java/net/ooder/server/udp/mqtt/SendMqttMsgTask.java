...
//延期异步执行
class SendMqttMsgTask implements Callable<Boolean> {
    private final JDSSessionHandle serverhandle;
    private final ClusterEvent cloneevent;
    private final String sysCode;
    private static final Log logger = LogFactory.getLog(JDSConstants.CONFIG_KEY, SendMqttMsgTask.class);
    private final static SerializeConfig config = new SerializeConfig();

    static {
        config.propertyNamingStrategy = PropertyNamingStrategy.SnakeCase;
    }
    SendMqttMsgTask(JDSSessionHandle serverhandle, ClusterEvent cloneevent, String sysCode) {
    ...
    public Boolean call() {
        ...
        try {
            client = JDSServer.getInstance().getJDSClientService(serverhandle, OrgConstants.CLUSTERCONFIG_KEY);
            ...
            if (cnnectionHandle instanceof AbstractUDPHandle) {
                ...
                if (client.getConnectInfo() != null && !portList.contains(key) && udpCnnectionHandle.getIp() != null) {
                    ...
                    String eventStr = JSONObject.toJSONString(cloneevent,config);
                        ...
                        switch (cloneevent.getDeadLine()) {
                            ...
                            case DELAY:
                                while (log.getStatus().equals(MsgStatus.NORMAL) && times < cloneevent.getMaxtimes()) {
                                    logger.info("msg timeort  =======serverhandle=" + serverhandle + " ip:port=" + ((AbstractUDPHandle) client.getConnectionHandle()).getIp() + ((AbstractUDPHandle) client.getConnectionHandle()).getPort());
                                    client.getConnectionHandle().send(eventStr);
                                    times = times + 1;
                                    try {
