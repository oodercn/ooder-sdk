ood.Class('ood.ModuleFactory', null, {
    Initialize: function () {
        ...
        ood.execTreeGrid = function (expression, params, isasy) {
            var result;
            if (!params) {
                params = {};
            }
            params.expression = expression;
            ood.Ajax('/LOCALESD/executeTreeGridCMD', params, function (txt) {
                var obj = txt;
                if (obj && obj.requestStatus != -1) {
                    result = obj.data;
                } else {
        ...
        //ood.showModule = function (cls, beforeShow, onEnd, threadid, cached, properties, events, parent, subId, left, top) {
        ood.showModule2 = function (cls, target, childname, params, data, title, parentModule) {
            if (!cls) {
                return;
            }
            if (childname == '') {
            ...
            var onEnd = function (url, remoteclass, test) {
                ...
                //this.getModule(cls, onEnd, threadid, false, properties, events);
                var clazz = ood.ModuleFactory.getModule(remoteclass, function () {
                    if (parentModule) {
