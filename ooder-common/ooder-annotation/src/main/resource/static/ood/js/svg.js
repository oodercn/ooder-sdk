ood.Class("ood.svg", "ood.UI",{
    Before:function(key, parent_key, o){
        ood.absBox.$type[key.replace("ood.","")]=ood.absBox.$type[key]=key;
        return true;
    },
    Initialize:function(){
...
//            "target":1,
        ...
        this.$attr={
            ...
            path:{x:1,y:1,cx:1,cy:1,path:1}
        };
        ood.each(this.$attr,function(o) {
            ood.merge(o,attr);
        });
    ...
    Instance:{
                        ...
                        if(!key || key===tag){
                            ...
                            if(node&&('raphaelid' in node)){
                                ...
                                if(paper && (node=paper.getById(node.raphaelid))){
                                    var attf=node.attr();
                                    ood.filter(attf,function(o,i){
                                        if(i=='transform' && ood.isArr(o) && o.length===0)o="";
        ...
        setAttr:function(key, attr, reset, notify){
            ...
            return this.each(function(prf){
                ...
                if(reset){
                    ...
                    ood.each(oAttr,function(o,i){
                        if(!(i in attr2)){
                            attr2[i]={};
                        }

                        ood.each(o,function(o1,i1){
                            if(!(i1 in attr2[i])){
                ...
                if(prf._elset){
                    prf._elset.forEach(function(el){
                        var key1=el.type,tag=prf.getKey(el.node.id,true);
                        if(ood.isHash(key) || key===tag){
                            node=el.node;
                            if(node&&('raphaelid' in node)){
                                var paper=prf.boxing().getPaper();
                                if(paper && (node=paper.getById(node.raphaelid))){
                                    var rattr=ood.copy(attr2[tag]);
                                    if(rattr){
                                        if('src' in rattr)
                                            rattr.src = ood.adjustRes(rattr.src);
                                        node.attr(rattr);
                                    }
                                }
                            }
                ...
                if(prf.box.ISCOMBO){
                    prop.attr=ood.merge(prop.attr,attr2,function(o,i){
                        if(typeof o=='object'){
                            if(i in prop.attr)
                                ood.merge(prop.attr[i], o, 'all');
                            else
                                prop.attr[i]=ood.copy(o);
                        }else{
                            return true;
                        }
                    });
                    ...
                    ood.each(prop.attr,function(o,i){
                        ood.filter(o,function(o1,i1){
                            if(i1=='transform' && ood.isArr(dftAttr[i1]) && dftAttr[i1].length===0)dftAttr[i1]="";
                            // special for text
                            if((prf.box.$tagName[i]|| i.split("_")[0].toLowerCase())=='text'){
                    ...
                    ood.merge(prop.attr, attr2.KEY, 'all');

                    // filter default setting
                    ood.filter(prop.attr,function(o1,i1){
                        if(i1=='transform' && ood.isArr(dftAttr[i1]) && dftAttr[i1].length===0)dftAttr[i1]="";
                        // special for text
        ...
        toFront:function(){
            return this.each(function(prf){
                ...
                if(prf._elset)
                    ood.arr.each(prf._elset.items,function(el){
                        el.toFront();
                    });
                var arr=ood.get(prf,['parent','children']);
                if(arr && arr.length){
                    var index=ood.arr.subIndexOf(arr,"0",prf),o;
                    if(index!=-1){
                        o=arr[index];
                        arr.splice(index,1);
                        arr.push(o);
        ...
        _getBBox:function(key, withTransform){
            ...
            if((el=prf._elset)&&el.length){
                el[0]._.dirty=1;
                bbox={};
                ood.merge(bbox,el[0]._getBBox(withTransform!==false),'all',true);
        ...
        _getConnectAnchors:function(){
            ...
            if(prf){
                if(ood.get(prf._pathCached,['_connAnchors']))
                    return ood.get(prf._pathCached,['_connAnchors']);
                ...
                if(ss&&ss[0]){
                    ...
                    if(prf._pathCached)ood.set(prf._pathCached,['_connAnchors'], rst);
        ...
        _getConnectPath:function(){
            ...
            if(ood.get(prf._pathCached,['_connPath']))
                return ood.get(prf._pathCached,['_connPath']);
            ...
            if(prf && (ss=prf._elset) && ss[0]){
                ...
                if(prf._pathCached)ood.set(prf._pathCached,['_connPath'], path);
        ...
        _getConnectPoint:function(anchor){
            if(anchor){
                ...
                if(ood.get(prf._pathCached,['_connPoint',anchor]))
                    return ood.get(prf._pathCached,['_connPoint',anchor]);
                ...
                if(prf._pathCached)ood.set(prf._pathCached,['_connPoint',anchor],rst);
                        ...
                        rect:function (el){
                            ...
                            handler.dot = createDDElem(r.circle(attr.x+attr.width/2, attr.y+attr.height/2).attr(dotAttr1).attr('cursor','move'),
                                el, function (x, y, ax, ay){
                                    ...
                                    ood.tryF(el.onShapeChanged,[nshape]);
                            ...
                            handler.dot1 = createDDElem(r.circle(attr.x, attr.y).attr(dotAttr2).attr('cursor','nw-resize'),
                                el, function (x, y, ax, ay){
                                    ...
                                    if(el.attr("r")>m){
                                        ...
                                        ood.tryF(el.onShapeChanged,[{r:m}]);
                                    }
                                    handler.dot3.attr({
                        ...
                        circle:function (el){
                            ...
                            handler.dot = createDDElem(r.circle(attr.cx, attr.cy).attr(dotAttr1).attr('cursor','move'),
                                el, function (x, y){
                                    ...
                                    ood.tryF(el.onShapeChanged,[pos]);
                                ...
                                addSegmentJunctionDD=function(r,el,x,y,index,type3,conType,prf){
                                    ...
                                    return createDDElem(circle, el, function (x, y, ax, ay, beForced, mx, my,ix,iy,event, callback) {
                                        var paper=el.paper,
                                            ox = this.attr('cx'),
                                            oy = this.attr('cy'),
                                            X = ood.isSet(ix)?ix:(ox + x),
                                            Y = ood.isSet(iy)?iy:(oy + y),
                                            pos,anchorPos;
                                        if(ood.isSet(ix)){
                                            x=X-ox;
                                        }
                                        ...
                                        if(conType=='from'||conType=='to'){
                                            ...
                                            prf._elset.hide();
                                            el._shape.hide();
                                            ood.arr.each(handlers,function(o,i){
                                                if(i=o.dot)i.hide();
                                                if(i=o.dot1)i.hide();
                                                if(i=o.dot2)i.hide();
                                                if(i=o.dot3)i.hide();
                                            ...
                                            if(esrc!==paper.canvas){
                                                var tobj=ood.UIProfile.getFromDom(esrc.id);
...
//console.log('first magnetic');
                                        ...
                                        var pathModified,
                                            pos = {cx: X, cy: Y};
                                        if(beForced || false!==ood.tryF(manualPathModify,[index,paths,X,Y,this,pos])){
                                ...
                                handlers=[];
                            if(ood.isStr(paths))paths=Raphael.parsePathString(paths);
                            ...
                            if(isConnector && connType=="flowchart"){
                                prf._ddstartFun=function(prf,index,dot){
                                    if(index===0 || index===prf._pathHandler.handlers.length-1)
                                        ood.arr.each(prf._pathHandler.handlers,function(o,i){
                    ...
                    start = function(mx,my,event){
                        ...
                        absPos = ood(this.paper.canvas.parentNode).offset(null);
                        ...
                        if(t=this._attached){
                            ...
                            // get prf
                            if(prf._ddstartFun)ood.tryF(prf._ddstartFun,[prf, this._handlerIndex,this]);
                        }
                        ...
                        if(t=this._attached){
                            ...
                            var attr=t.attr();
                            if(isCombo){
                                ood.merge(prf.properties.attr.KEY,attr,'all');
                                attr=prf.properties.attr.KEY;
                            }else{
                                ood.merge(prf.properties.attr,attr,'all');
                            ...
                            // get prf
                            if(prf._ddendFun)ood.tryF(prf._ddendFun,[prf,this._handlerIndex,this]);
                    ...
                    createDDElem=function(obj,el,update,key,index){
                        ...
                        if(update){
                            ...
                            obj.click(function(event){
                                ...
                                ood.Event.stopBubble(event);
                                ood.Event.stopDefault(event);
                            });
                            obj.contextmenu(function(event){
                                if(callback)callback(this, event, 'contextmenu', el, key, index);
                            });
                            // stop select parent
                            //ood(obj.node).onContextMenu(function(){
                            //    return false;
                            //});
                        }
                        obj.toFront();
                    ...
                    moveFun=function (el, x, y, ax, ay, limitX1, limitX2, limitY1, limitY2){
                        switch(el.type){
                            ...
                            case 'circle':{
                                ...
                                ood.tryF(el.onShapeChanged,[rst]);
                                rst.dx=ax-lx;
                                rst.dy=ay-ly;
                                return rst;
                            }
    ...
    Static:{
            ...
            animDraw:{
                ...
                action:function(v){
                    ...
                    var length = elm.getTotalLength(), 
                        style=elm.style,
                        trans=function(v){style.transition = style[ood.browser.cssTag2+'Transition'] =v;},
                    ...
                    if(prf._flowthread)ood.Thread.abort(prf._flowthread);
                    ood.setTimeout(function(t){
            ...
            offsetFlow:{
                ...
                action:function(v){
                    ...
                    if(v){
                        ...
                        arr=ood(elm.node).attr('stroke-dasharray');
                        if(!arr)return;
                        ...
                        if(sum>0){
                            ...
                            if(speed){
                                prf._flowthread=ood.Thread.repeat(function(){
        ...
        Behaviors:{
            ...
            onContextmenu:function(profile, e, src){
                if(profile.onContextmenu)
                    return profile.boxing().onContextmenu(profile, e, src,ood.Event.getPos(e))!==false;
            }
        },
        EventHandlers:{
            onClick:function(profile, e, src){},
            onDblClick:function(profile, e, src){},
            onContextmenu:function(profile, e, src,pos){},
            onHotKeydown:null,
            onHotKeypress:null,
            onHotKeyup:null
        },
        _beforeSerialized:function(profile){
            ...
            prop.attr=ood.clone(prop.attr,true);
            ood.filter(prop.attr,function(o,i){
                if(i=='transform'&&(!o||o.length<1))return false;
                if(i=='href'||i=='target')return false;
                if(i=='path' && ood.isArr(prop.attr.path))prop.attr.path=o.join('');
            });
            return o;
        },
        ...
        _adjustAttr:function(attr){
            ...
            ood.each(attr,function(o,i){
                if(/^[A-Z_]+(_[0-9]+)?$/.test(i) && ood.isHash(o) && ((i.indexOf("_")!=-1)||( i in ns.$tagName))){
                    attr2[i]=attr2[i]||{};
                    ood.each(o,function(o1,i1){
        ...
        $adjustBB:function(key,value){
            ...
            ood.filter(bb,function(v,k){
                if(ood.arr.indexOf(keys,k)==-1 || v==='auto' || v==='')return false;
                if(!ood.isNumb(v)){
                    bb[k]=parseFloat(v)||0;
                    return false;
                }
        ...
        $transform:function(opath,trans){
            if(!ood.isStr(opath))opath=opath.join('');
        ...
        $setBB:function(prf,type,bbox,attr,el,notify){
            ...
            switch(type){
                case 'circle':{
                    if(ood.isNumb(bbox.width))
                        h.r=bbox.width/2;
                    if(ood.isNumb(bbox.height)){
                        if(bbox.height/2>=attr.r && (ood.isSet(h.r)?h.r>=attr.r:1))
                            h.r=ood.isSet(h.r)?Math.max(h.r,bbox.height/2):(bbox.height/2);
                        else
                            h.r=ood.isSet(h.r)?Math.min(h.r,bbox.height/2):(bbox.height/2);
                    }
                    h.cx=ood.isNumb(bbox.x)?(bbox.x+(ood.isSet(h.r)?h.r:attr.r)):(attr.cx+(ood.isSet(h.r)?(h.r-attr.r):0));
                    h.cy=ood.isNumb(bbox.y)?(bbox.y+(ood.isSet(h.r)?h.r:attr.r)):(attr.cy+(ood.isSet(h.r)?(h.r-attr.r):0));
                }break;
                case 'ellipse':{
                    if(ood.isNumb(bbox.width))
                        h.rx=bbox.width/2;
                    if(ood.isNumb(bbox.height))
                        h.ry=bbox.height/2;
                    h.cx=ood.isNumb(bbox.x)?(bbox.x+(ood.isSet(h.rx)?h.rx:attr.rx)):(attr.cx+(ood.isSet(h.rx)?(h.rx-attr.rx):0));
                    h.cy=ood.isNumb(bbox.y)?(bbox.y+(ood.isSet(h.ry)?h.ry:attr.ry)):(attr.cy+(ood.isSet(h.ry)?(h.ry-attr.ry):0));
                }break;
                case 'rect':
                case 'image':{
                    if(ood.isNumb(bbox.x))
                        h.x=bbox.x;
                    if(ood.isNumb(bbox.y))
                        h.y=bbox.y;
                    if(ood.isNumb(bbox.width))
                        h.width=bbox.width;
                    if(ood.isNumb(bbox.height))
                ...
                case 'text':{
                    ...
                    /*}else{
                        if(ood.isNumb(obbox.x))
                            h.x=obbox.x;
                        if(ood.isNumb(obbox.y))
                            h.y=obbox.y;
                        if(ood.isNumb(obbox.width))
                            h.width=obbox.width;
                        if(ood.isNumb(obbox.height))
                ...
                case 'path':{
                    ...
                    if(('width' in h && obbox.width!==h.width)||('height' in h && obbox.height!==h.height)){
                        var ww=('width' in h)?h.width:null,
                            ...
                            npath=ood.svg.$transform(opath, "s"+((ood.isSet(ww)&&ww!==obbox2.width)?(obbox2.width===0?(ww>=0?1.1:0.9):ww/obbox2.width):"1")+","+((ood.isSet(hh)&&hh!==obbox2.height)?(obbox2.height===0?(hh>=0?1.1:0.9):hh/obbox2.height):"1")+","+obbox.x+","+obbox.y);
                        h.path=npath;
                    }
                }break;
            }

            var attr2=ood.svg.$attr[type];
            ood.filter(h,function(o,i){
                if(ood.isSet(o) && (i in attr2)){
                    attr[i]=o;
                }else{
                    return false;
                }
        ...
        _syncConnectors:function(prf){
            ...
            if(prf.parent.key!=="ood.UI.SVGPaper")return;

            var children=ood.get(prf,['parent','children']),
                alias=prf.alias;
            if(children&&children.length){
                ood.arr.each(children,function(o,i){
                    if(!o[0].destroyed && prf.renderCompleted && o[0].renderCompleted && o[0].key==="ood.svg.connector"){
                        if(alias===ood.get(o[0],['properties','fromObj'])){
                            // redraw from point
                            o[0].box._reconnect(o[0],prf);
                        }
                        if(alias===ood.get(o[0],['properties','toObj'])){
                            // redraw to point
        ...
        _syncAlias:function(prf,oa,na){
            ...
            if(children&&children.length){
                ood.arr.each(children,function(o,i){
                    if(o[0].key==="ood.svg.connector"){
                        if(oa===ood.get(o[0],['properties','fromObj'])){
                            ood.set(o[0],['properties','fromObj'],na);
                        }
                        if(oa===ood.get(o[0],['properties','toObj'])){
                            ood.set(o[0],['properties','toObj'],na);
                        }
                    }
                });
            }
...
ood.Class("ood.svg.circle", "ood.svg",{
    Instance:{
        _setBBox:function(key,value){
            var bb=ood.svg.$adjustBB(key,value);
            return this.each(function(prf){
                ood.svg.$setBB(prf,'circle', bb, prf.properties.attr, prf._elset&&prf._elset[0]);
            });
        }
    },
    Static:{
...
ood.Class("ood.svg.ellipse", "ood.svg",{
    Instance:{
        _setBBox:function(key,value){
            var bb=ood.svg.$adjustBB(key,value);
            return this.each(function(prf){
                ood.svg.$setBB(prf,'ellipse', bb, prf.properties.attr, prf._elset&&prf._elset[0]);
            });
        }
    },
    Static:{
...
ood.Class("ood.svg.rect", "ood.svg",{
    Instance:{
        _setBBox:function(key,value){
            var bb=ood.svg.$adjustBB(key,value);
            return this.each(function(prf){
                ood.svg.$setBB(prf,'rect', bb, prf.properties.attr, prf._elset&&prf._elset[0]);
            });
        }
    },
    Static:{
...
ood.Class("ood.svg.image", "ood.svg",{
    Instance:{
        _setBBox:function(key,value){
            var bb=ood.svg.$adjustBB(key,value);
            return this.each(function(prf){
                ood.svg.$setBB(prf,'image', bb, prf.properties.attr, prf._elset&&prf._elset[0]);
            });
        }
    },
    Static:{
        ...
        _draw:function(paper, prf, prop){
            prop=prop.attr;
            var el = paper.image(ood.adjustRes(prop.src),prop.x,prop.y,prop.width,prop.height);
            el.node.id=prf.box.KEY+":"+prf.serialId+":";
            return paper.set().push(el);
        }
    }
});
ood.Class("ood.svg.text", "ood.svg",{
    Instance:{
        _setBBox:function(key,value){
            var bb=ood.svg.$adjustBB(key,value);
            return this.each(function(prf){
                ood.svg.$setBB(prf,'text', bb, prf.properties.attr, prf._elset&&prf._elset[0]);
            });
        }
    },
    Static:{
        ...
        _draw:function(paper, prf, prop){
            ...
            if(ood.isSet(prop.text))el.attr('text',prop.text);
            el.node.id=prf.box.KEY+":"+prf.serialId+":";
            return paper.set().push(el);
        }
    }
});
ood.Class("ood.svg.path", "ood.svg",{
    Instance:{
        getPath:function(){
        ...
        _setBBox:function(key,value){
            var bb=ood.svg.$adjustBB(key,value);
            return this.each(function(prf){
                ood.svg.$setBB(prf,'path', bb, prf.properties.attr, prf._elset&&prf._elset[0]);
            });
        }
    },
    Static:{
        ...
        _draw:function(paper, prf, prop){
            ...
            var x=prop.x?prop.x:0;
            var y=prop.y?prop.x:0;
            var paths=Raphael.parsePathString(prop.path);
            for(var i=0, ii=paths.length, l; i<ii; i++) {
                l = paths[i].length;
                if (l != 'Z') {
                    paths[i][1] += x;
...
ood.Class("ood.svg.absComb", "ood.svg",{
    Instance:{
        _setBBox:function(key,value,notify){
            var bb=ood.svg.$adjustBB(key,value);
            return this.each(function(prf){
                var prop=prf.properties, ss=prf._elset;
                ood.svg.$setBB(prf,prf.box._type, bb, prop.attr.KEY, ss&&ss[0]);

                if(prf.renderId && ss&&ss[0]){
                    if(prop.attr.TEXT&&prop.attr.TEXT.text){
                        var attr=ood.copy(prop.attr.TEXT||{}),att2=ss[0]._getBBox(true);
                        ...
                        ood.svg.$setBB(prf,"text", bb, attr, ss&&ss[1],notify);
                    }
                }
            });
        },
            ...
            return this.each(function(prf){
                ...
                if(prf.renderId && ss && ss[0]){
                    if(prop.attr.TEXT && prop.attr.TEXT.text){
                        ...
                        attr.hAlign=prop.hAlign;
                        attr.vAlign=prop.vAlign;
                        ood.svg.$setBB(prf,"text", att2, attr, ss[1], false);
                    }
                }
            });
        },
    ...
    Static:{
        ...
        _beforeSerialized:function(profile){
            var o = arguments.callee.upper.call(this,profile),prop=o.properties;
            prop.attr=ood.clone(prop.attr,true);
            ood.filter(prop.attr.KEY,function(o,i){
                if(i=='transform'&&(!o||o.length<1))return false;
                if(i=='href'||i=='target')return false;
                if(i=='path' && ood.isArr(prop.attr.KEY.path))prop.attr.KEY.path=o.join('');
            });
            if(prop.attr.TEXT){
                ood.filter(prop.attr.TEXT,function(o,i){
...
ood.Class("ood.svg.rectComb", "ood.svg.absComb",{
    Static:{
        _type:'rect',
        Templates:{
            tagName:'rect',
...
ood.Class("ood.svg.circleComb", "ood.svg.absComb",{
    Static:{
        _type:'circle',
        Templates:{
            tagName:'circle',
...
ood.Class("ood.svg.ellipseComb", "ood.svg.absComb",{
    Static:{
        _type:'ellipse',
        Templates:{
            tagName:'ellipse',
...
ood.Class("ood.svg.pathComb", "ood.svg.absComb",{
    Static:{
        _type:'path',
        DataModel:{
            attr:{
        ...
        _drawRoot:function(paper,attr){
            ...
            if ((attr.cx &&attr.cx!=attr.x) ||(attr.cy&&attr.cy!=attr.y)){
                var x=attr.cx?attr.cx-attr.x:0;
                var y=attr.cy?attr.cy-attr.y:0;
                var paths=Raphael.parsePathString(attr.path);
                for(var i=0, ii=paths.length, l; i<ii; i++){
                    l = paths[i].length;
                    switch (paths[i][0]) {
...
ood.Class("ood.svg.imageComb", "ood.svg.absComb",{
    Static:{
        IMGNODE:1,
        $noShape:1,
        _type:'image',
...
ood.Class("ood.svg.connector","ood.svg.absComb",{
    Instance:{
    ...
    Static:{
        ...
        _syncAttr:function(prf,options,shapeChanged){
            ...
            if(('bBox' in options) && prf.parent){
                if(prop.fromObj&&prop.fromPoint){
                    ood.arr.each(prf.parent.children,function(o){
        ...
        _redrawBrokenLine:function(prf,inputPoint,x,y){
            ...
            if(startConn){
                ins=null;
                //startPoints 3
                ood.arr.each(prf.parent.children,function(p){
                    if(p[0].alias==prop.fromObj){
                        ...
                        if(sPoint){
                            ...
                            if(sBox=ood.get(p[0]._pathCached,['_connBBox'])){
                            }else{
                                sBox=Raphael.pathBBox(sPath);
                                ood.set(p[0]._pathCached,['_connBBox'], sBox)
                            }
                ...
                ood.arr.each(prf.parent.children,function(p){
                    if(p[0].alias==prop.toObj){
                        ...
                        if(ePoint){
                            ...
                            if(eBox=ood.get(p[0]._pathCached,['_connBBox'])){
                            }else{
                                eBox=Raphael.pathBBox(ePath);
                                ood.set(p[0]._pathCached,['_connBBox'], eBox)
                            }
            ...
            ood.arr.each(startPoints,function(sp,i){
                ood.arr.each(endPoints,function(ep,j){
                    for(var k=0;k<=1;k++){
                        path=ood.clone(startPath[i],true);
                        if(k===0){
                        ...
                        if(endPath[j] && endPath[j].length)ood.arr.insertAny(path, endPath[j]);
            ...
            ood.arr.each(paths,function(o){
                inter=0;
                var cx,cy;
                if(sBox||eBox){
                    cx=o.path[0][1];cy=o.path[0][2];
                    ood.arr.each(o.path,function(p,i){
                        if(i!==0){
                            if(p[0]=='H'){
			...
			// < 22, stable sort
            ood.arr.stableSort(paths,function(x,y){
...
ood.Class("ood.svg.group", "ood.svg.absComb",{
    Instance:{
        _adjustText:function(){
            return this;
        },
    ...
    Static:{
        ...
        _beforeSerialized:function(profile){
            var o = arguments.callee.upper.call(this,profile),prop=o.properties;
            var attrs=prop.attr=ood.clone(prop.attr,true);
            
            ood.filter(attrs.KEY,function(o,i){
                if(i=='transform'&&(!o||o.length<1))return false;
                if(i=='href'||i=='target')return false;
                if(i=='path' && ood.isArr(prop.attr.KEY.path))prop.attr.KEY.path=o.join('');
            });
            // other elements
            ood.each(attrs,function(attr,key){
                if(key=="KEY")return;
                ood.filter(attr,function(o,i){
                ...
                if(type=="path"){
                    ood.each(attr,function(o,i){
                        if(i=='path' && ood.isArr(o))attr.path=o.join('');
                    });
                }
            });
